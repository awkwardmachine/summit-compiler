...
llvm::Value* CodeGen::codegen(AST::MemberAssignmentStmt& stmt) {
    return StatementCodeGen::codegenMemberAssignment(*this, stmt);
}
...
std::string wrapperSource = 
    ...
    "    ss << std::setfill('0');\n"
    "    \n"
    "    // Format: YYMMDD-HHMM (Year, Month, Day - Hour, Minute)\n"
    "    ss << std::setw(2) << (tm.tm_year + 1900) % 100;r\n"
    ...
    "\n"
    "void printBanner() {\n"
    "    std::cout << \"Summit Compiler v0.0.1 for x64 architecture\" << std::endl;\n"
    "    std::cout << \"Copyright (C) 2025 awkwardmachine. All rights reserved.\" << std::endl;\n"
    "    std::cout << std::endl;\n"
    "    std::cout << \"Build date: \" << getBuildTimestamp() << std::endl;\n"
    "    std::cout << \"Build number: \" << getBuildNumber() << std::endl;\n"
